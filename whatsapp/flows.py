...
def _start_voice_quote(inbound: InboundMessage, deps: FlowDeps) -> None:
    ...
    if deps.debounce_window_s > 0:
        ...
        deps.send_buttons(
            phone,
            "⏳ Message vocal enregistré. Envoyez un autre vocal ou générez le devis.",
            [
                {"id": "debounce:flush", "title": "✅ Générer le devis"},
                {"id": "debounce:cancel", "title": "❌ Annuler"},
            ],
        )
        return
...
def _handle_interactive(inbound: InboundMessage, deps: FlowDeps) -> None:
    ...
    # Phase 3: Debounce flush/cancel buttons
    if rid == "debounce:flush":
        flushed = _maybe_flush_debounce(user_id, phone, deps, force=True)
        if not flushed:
            deps.send_text(phone, "Aucun vocal en attente. Envoyez un nouveau vocal.")
        return
    if rid == "debounce:cancel":
        # Clear all pending voices without processing
        if deps.fetch_pending_fn and deps.mark_processed_fn:
            pending = deps.fetch_pending_fn(user_id, deps.debounce_window_s * 60)
            if pending:
                deps.mark_processed_fn([p.get("wamid") for p in pending])
        deps.send_text(phone, "🗑️ Vocaux annulés. Envoyez un nouveau vocal pour recommencer.")
        return
...
def _apply_edit(user_id: int, phone: str, inbound: InboundMessage, deps: FlowDeps) -> None:
    ...
    if inbound.msg_type == "audio":
        ...
        try:
            ...
            logger.exception("[flows] edit voice transcribe failed")
            deps.send_text(phone, f"❌ Erreur transcription : {type(exc).__name__}")
            return
    else:
        text = inbound.payload.get("body", "")
