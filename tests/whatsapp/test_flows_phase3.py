...
class TestDebounceWindow:
    ...
    def test_debounce_voices_15s_apart_processed_separately(self, tmp_path):
        ...
        # fetch_pending_fn returns only the current message (simulates older ones were processed)
        def mock_fetch(uid, window_s):
            # Always return just one row: the current voice being processed
            # In real scenario, older ones would be cleared after processing
            return []  # Return empty to simulate they've been processed
        ...
        with patch.object(flows_module._quote, "quick_extract", side_effect=spy_extract):
            ...
            # Second voice (simulates 15s later, after first was already processed)
            inbound2 = _inbound("audio", {"id": "media-456"}, wamid="wamid.2")
            flows_module._start_voice_quote(inbound2, deps)

        # Verify separate processing: should have 2 distinct calls with different content
        assert len(extract_calls) >= 2
        assert extract_calls[0] == "premier message"
        assert extract_calls[1] == "deuxième message"
